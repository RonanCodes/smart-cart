...
export function EmailStep({
  ...
  async function verify(e: React.FormEvent) {
    ...
    // OTP verified at the end of onboarding: the account is created + signed in.
    // Same funnel event as the advertised /sign-in, tagged by source so the two
    // login entries stay distinguishable.
    ...
    <div className="flex flex-col gap-4" data-testid="onboarding-email-step">
      {substep === 'email' ? (
        ...
        <form onSubmit={verify} className="space-y-3">
          ...
          <Input
            inputMode="numeric"
            // Focus the code field the moment it appears (right after the code
            // email is sent) so the keyboard pops straight up.
            autoFocus
