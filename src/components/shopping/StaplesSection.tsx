...
export function StaplesSection({
  ...
  /**
   * Fired whenever the saved staples change (add / remove). The route lifts this
   * up so the price comparison + the single cart action include the extras and
   * recompute when one is added or removed (#311). Optional, so the empty-state
   * usage (no comparison below it) can omit it.
   */
