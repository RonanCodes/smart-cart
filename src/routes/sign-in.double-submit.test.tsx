 ...
 *
 * Same defect and same fix as LoginForm; this locks the /sign-in route component
 * so the two entries can never drift apart again.
 */
...
describe('SignIn route: no double-submit while the session is being confirmed', () => {
  it('keeps the submit button disabled until the navigation is issued', async () => {
    ...
    releaseConfirm(true)
    await waitFor(() => expect(window.location.href).toBe('/week'))
  })
